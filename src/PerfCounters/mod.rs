...
impl PerfMetrics {
    ...
    pub fn start_counters_4_cpi(&mut self, process_pid: u32) {
        ...
        let temp_4_meas = MeasuredCounters {
            value_unhalted_core: self.pc_unhalted_core
                .as_mut()
                .unwrap()
                .read()
                .expect("Can not read counter"),
            value_ret_instructions: self.pc_ret_instructions
                .as_mut()
                .unwrap()
                .read()
                .expect("Can not read counter"),
    ...
    pub fn get_cpi(&mut self) -> f64 {
        ...
        let cpi: f64 =
            ((self.final_values.as_mut().unwrap().value_unhalted_core -
              self.initial_values.as_mut().unwrap().value_unhalted_core) /
             (self.final_values.as_mut().unwrap().value_ret_instructions -
              self.initial_values.as_mut().unwrap().value_ret_instructions)) as f64;
        return cpi;
    }
}
