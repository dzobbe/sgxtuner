...
impl EnergyEval {
	...
	**/
    pub fn execute_test_instance(&mut self, params: &HashMap<String, u32>, energy_type: EnergyType) -> Option<f64> {
		...
		for i in 0..num_iterations {
			...
			println!("{} {}", Green.paint("====> Iteration Number for current Configuration: "), i+1);
	        ...
	        ****/
			...
			target_alive = match TcpStream::connect(("127.0.0.1", target_port)){
				Err(e) => {
					println!("{} The Target Application seems down. Maybe a bad configuration: {}", Red.paint("*****ERROR***** --> "), e);
	        ...
	        ****/
			let nrg: f64=match energy_type{
				EnergyType::throughput => {
			        ...
			        println!("{} {:.4} KB/s", Red.paint("====> Response Rate: "), resp_rate);
			        resp_rate
				},
				EnergyType::latency	   => {
					//Latency Evaluation
		...
		if target_alive{
			...
			match energy_type{
					EnergyType::throughput => {
				        println!("{} {:.3} KB/s", Red.paint("====> Avg. Response Rate: "), avg_nrg);//Red.paint("Std. Dev.: "),std_dev);
					},
					EnergyType::latency	   => {
